...
#define UIP_TCP_SERVER_PORT1 81
...
static struct tcp_socket uip_tcp_socket1;
static uint8_t uip_tcp_rx_buf1[UIP_TCP_BUFFER_SIZE_RX];
static uint8_t uip_tcp_tx_buf1[UIP_TCP_BUFFER_SIZE_TX];
...
static int uip_tcp_rx1(struct tcp_socket *s, void *ptr, const char *rxbuf, int rxlen)
{
	/* loopback */
	printf("%s", rxbuf);
	tcp_socket_send_str(&uip_tcp_socket1, rxbuf);
	return 0;
}
...
static void uip_tcp_event1(struct tcp_socket *s, void *ptr, tcp_socket_event_t event)
{
	/* XXX do something? */
#ifdef UIP_DEBUG
	printf("uip_tcp_event1 %d\n", event);
#endif
}
...
int main(void)
{
	...
	tcp_socket_register(&uip_tcp_socket1, NULL,
		uip_tcp_rx_buf1, UIP_TCP_BUFFER_SIZE_RX,
		uip_tcp_tx_buf1, UIP_TCP_BUFFER_SIZE_TX,
		(tcp_socket_data_callback_t) uip_tcp_rx1, uip_tcp_event1);
	tcp_socket_listen(&uip_tcp_socket1, UIP_TCP_SERVER_PORT1);
	printf("Listening on %d\n", UIP_TCP_SERVER_PORT1);
